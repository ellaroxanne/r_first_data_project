...
So, code for my favorite number would be:

```{r ellasNum, exercise = TRUE}
...
```{r, myVar, exercise = TRUE}
```

Now, let's double-check that variable saved correctly. 
A straightforward way to do this with simple variables is the <code> print </code> function.

```{r, printFunc, exercise = TRUE, exercise.setup = "ellasNum"}
print(ellas_favorite_number)
```

Return to the exercise and try it with yours!
...
Let's start by trying to make a variable with the link we have, https://raw.githubusercontent.com/ellaroxanne/r_first_data_project/refs/heads/main/data_and_metadata/BLOY_morphology_AlaskaBC.csv. 

This is the link that was contained within the button in the beginning of this lesson
<!-- Have them copy this code, that's down below-->

```{r firstTry, exercise = TRUE}
first_try <- "https://raw.githubusercontent.com/ellaroxanne/r_first_data_project/refs/heads/main/data_and_metadata/BLOY_morphology_AlaskaBC.csv"
```
...
```{r printFirst, exercise = TRUE, exercise.setup = "firstTry"}
print(first_try)
```
...
```{r secondTry, exercise = TRUE, exercise.reveal_solution = TRUE}

```

```{r secondTry-solution, exercise.reveal_solution = TRUE}
...
```{r printData, exercise.reveal_solution = TRUE, exercise=TRUE, exercise.setup = "secondTry-solution"}

```

```{r, printData-code-check}
grade_code()
```

<div id="printData-hint">
**Hint:** We're already tried this, try looking at the last time we tried print()
</div>
...
```{r headFunction, exercise.reveal_solution = TRUE, exercise=TRUE, exercise.setup = "secondTry-solution"}
head(raw_data)
```
...
<button onclick="document.getElementById('section-q1-ans').style.display='block'">Reveal Answer</button>
<div id="q1-ans" style="display:none;">
  <p>By using the head() function, we can peek at the first few columns of data, giving us an idea of the varibles included in this data set.</p>
</div>
