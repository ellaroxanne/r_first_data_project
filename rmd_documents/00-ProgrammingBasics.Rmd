...
<!-- Doodle of Mary saying she's 22 --> 
...
Great! Since it printed 22, we know that <code> mary_age </code> is saved correctly. 

How old will she be in 5 years? You can probably do this in your head, but let's see if the computer can do it. 

```{r, first_var, exercise=TRUE, exercise.setup = "prepare-variables", eval=TRUE, echo= TRUE}
print(mary_age + 5)
```

Notice that it automatically computed the number [CHANGE THE WORDING] based on the value of our variable <code> mary_age </code>. 
...
<!-- Mary doodle, she lied about her age! She's actually 55. -->
...
Copy and paste the same code from the earlier block, where we computed Mary's age in five years. 
...
<!-- Maybe add an exercise here, assign your own variable for Mary's height in inches. Print it to make sure it's correct. Just make sure they create a float/whatever it's called and print it.--> 
